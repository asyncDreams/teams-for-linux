...
- **Graph API Enhanced Features** ([research](../research/graph-api-integration-research.md)): calendar surface shipped as a next-meeting tray tooltip (`graphApi.nextMeeting.*`, off by default); remaining work is delta-query sync, a richer calendar surface, and mail preview notifications. Foundation hardening shipped: client resilience (429/`Retry-After`, idempotent-only 5xx retry, one-shot 401 token refresh), presence hybrid with Graph 403 downgrade, and optional calendar-as-presence-provider. Presence 403 on tenants without `Presence.Read` consent is expected and handled.
