...
- **Graph API Enhanced Features** ([research](../research/graph-api-integration-research.md)): calendar surface (next-meeting tray or widget — the `graph-api-get-calendar-view` channel and preload bridge exist and lack a consumer), delta-query sync, mail preview notifications. Foundation hardening shipped: client resilience (429/`Retry-After`, idempotent-only 5xx retry, one-shot 401 token refresh), presence hybrid with Graph 403 downgrade, and optional calendar-as-presence-provider. Presence 403 on tenants without `Presence.Read` consent is expected and handled.
