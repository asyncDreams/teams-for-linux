...
- **Config Schema as Single Source of Truth** ([#2597](https://github.com/IsmaelMartinez/teams-for-linux/issues/2597)): phases 0-2 shipped in v2.12.0 (drift fixes, generated reference + `config-schema.json` with CI drift guard, docs config explorer); 3a (`applyMode` + nested-field metadata, hard-failing generator lint) and 4 (warn-only startup validation) landed; 3b (in-app settings window) implemented as `app/settings/` (Settings → Configuration…): schema-driven UI, validated writes persisted through the `legacyConfigStore` override-on-boot layering, live options reusing the `config-changed` path, restart action for the rest ([research](../research/documentation-and-config-ux-research.md)).
...
- **Graph API Enhanced Features** ([research](../research/graph-api-integration-research.md)): calendar surfaces shipped as a next-meeting tray tooltip (`graphApi.nextMeeting.*`) and a standalone calendar panel fed by incremental delta-query sync (`app/graphApi/calendarDeltaSync.js`) with meeting quick actions (accept/tentative/decline via `calendar-panel-respond`); mail preview notifications ship opt-in via `graphApi.mailPreview.*`. All off by default; the only remaining item is the settings UI (parked on [#2597](https://github.com/IsmaelMartinez/teams-for-linux/issues/2597)). Foundation hardening shipped: client resilience (429/`Retry-After`, idempotent-only 5xx retry, one-shot 401 token refresh), presence hybrid with Graph 403 downgrade, and optional calendar-as-presence-provider. Presence 403 on tenants without `Presence.Read` consent is expected and handled.
