...
| `meetupJoinPopOutWindow` | `boolean` | `false` | Open meeting joins in a dedicated meeting window (like native Teams) instead of navigating the main window. The main window stays on chat/calendar while the meeting runs in its own window | `restart` |
