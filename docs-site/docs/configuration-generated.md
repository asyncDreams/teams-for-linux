...
| `linux` | `object` | `{"mediaControls":{"enabled":false}}` | Linux desktop integration. mediaControls is an opt-in Linux media-control bridge. | `restart` |
...
| `meetupJoinRegEx` | `string` | `"^https://teams\\.(?:cloud\\.microsoft\|microsoft\\.com\|live\\.com)/(v2/\\?meetingjoin=\|meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)"` | Regex for Teams meetup-join and related links | `restart` |
...
| `meetupJoinPopOutWindow` | `boolean` | `true` | Open meeting and call joins in a dedicated window (like native Teams) instead of navigating the main window. The main window stays on chat/calendar while the meeting runs in its own window. Shares the main window's session, so sign-in carries over. Set to false to restore the single-window behaviour | `restart` |
...
| `msTeamsProtocols` | `object` | `{"v1":"^msteams:/(?:meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)","v2":"^msteams://teams\\.(?:cloud\\.microsoft\|microsoft\\.com\|live\\.com)/(?:meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)"}` | Regular expressions for Microsoft Teams protocol links (v1 and v2). Derived from the Teams host table in defaults.js — host set is not configured directly here. | `restart` |
| `url` | `string` | `"https://teams.cloud.microsoft"` | Microsoft Teams URL | `restart` |
| `hosts` | `object` | `{"autoRedirect":true}` | Teams host handling. autoRedirect: when true (the default), navigating to a legacy Teams host (teams.microsoft.com / teams.live.com) is normalized to the canonical host teams.cloud.microsoft, preserving path, search, and hash. Set false to keep legacy hosts verbatim — useful if a conditional-access rule is pinned to the old host. MCAS-proxied hosts (*.mcas.ms) are never rewritten. | `restart` |
...
| `extensions` | `object` | `{"enabled":false,"allowUnpacked":true,"allowCrx":true,"developerMode":false,"preload":[],"identityShim":{"enabled":false,"allowedRedirectHosts":["chromiumapp.org"],"authCompleteHosts":["otter.ai"]}}` | Chromium extension support (Otter.ai, Grammarly, Loom, Microsoft Editor). enabled: master flag (off by default). allowUnpacked: offer Load unpacked in the Extensions manager. allowCrx: offer CRX installation after validation. developerMode: show advanced manifest and developer actions in the manager. preload: absolute paths to unpacked extension directories loaded at startup (one string per extension). Installed CRX files are copied under &lt;userData&gt;/extensions and restored after restart. Requires restart for preload changes. | `restart` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `linux.mediaControls.enabled` | `boolean` | `false` | Expose Linux media/call controls through the desktop integration bridge. Off by default. |
...
| `notifications.grouping` | `boolean` | `false` | Coalesce notifications by conversation/channel via tag (when true, notifications from the same chat/channel replace each other). Off by default for one beta. |
| `notifications.actions` | `boolean` | `false` | Show actionable buttons (Reply / Mark as read / Open / Join) on Electron notifications where supported. Off by default for one beta. |
| `notifications.avatar` | `boolean` | `false` | Fetch and display sender avatar in notifications when available (uses notification payload icon or Graph People photo). Off by default. |
| `notifications.suppressInApp` | `boolean` | `true` | Hide Teams' built-in in-app toast/banner when an OS notification is shown (prevents double toast). On by default. |
| `notifications.history.enabled` | `boolean` | `false` | Persist a local notification timeline and expose it through View &gt; Notification History. Off by default. |
...
| `msTeamsProtocols.v2` | `string` | `"^msteams://teams\\.(?:cloud\\.microsoft\|microsoft\\.com\|live\\.com)/(?:meet/\|l/(?:app\|call\|channel\|chat\|entity\|file\|meet(?:ing\|up-join)\|message\|task\|team)/)"` | Regular expression matching host-based msteams:// scheme protocol links. |

### hosts

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `hosts.autoRedirect` | `boolean` | `true` | Normalize legacy Teams hosts to the canonical host on navigation. When false, legacy hosts are left as-is. |

### presence

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `presence.graphPoll.enabled` | `boolean` | `false` | Enable Graph presence polling as a hybrid correction layer on top of DOM scraping; requires graphApi.enabled and Presence.Read consent, otherwise polls are skipped gracefully. |
| `presence.graphPoll.intervalMs` | `number` | `60000` | Poll interval in milliseconds for Microsoft Graph /me/presence when graphPoll is enabled. |
| `presence.sync.enabled` | `boolean` | `false` | Enable the unified presence reconciliation layer. Off by default; DOM and Graph behavior remains unchanged when disabled. |
...
| `presence.sync.calendar.enabled` | `boolean` | `false` | Poll the Microsoft Graph calendar as an optional presence provider; requires Graph API access. |
| `presence.sync.calendar.preBusy` | `boolean` | `false` | Derive Busy shortly before an upcoming calendar event. |
| `presence.sync.calendar.reminderMinutes` | `number` | `5` | Minutes before a calendar event when the optional pre-busy state starts. |
| `presence.sync.calendar.pollIntervalMs` | `number` | `60000` | Calendar provider poll interval in milliseconds. |
| `presence.keepAlwaysOnline` | `boolean` | `false` | Legacy alias for keepAlwaysOnlineMode=always. Kept for backwards compatibility with existing configuration files. |
| `presence.keepAlwaysOnlineMode` | `string` | `"disabled"` | Presence preservation mode. Disabled does not inject activity, Always maintains Available continuously, and Business Hours follows presence.businessHours. |
| `presence.businessHours.enabled` | `boolean` | `false` | Enable the configured business-hours schedule when keepAlwaysOnlineMode is business-hours. |
...
| `presence.businessHours.weekdays` | `array` | `[1,2,3,4,5]` | ISO weekdays enabled for business-hours mode: 1 Monday through 7 Sunday. |
| `presence.businessHours.timezone` | `string` | `""` | IANA timezone for the business-hours schedule; empty uses the operating system timezone. |
| `presence.smartPresence` | `boolean` | `false` | Yield keep-online activity injection while in a meeting/call, presenting, screen sharing, DND, or explicit Busy. Off by default. |
...
| `media.showStatusOnTrayIcon` | `boolean` | `false` | Overlay the user presence status as a dot on the tray icon on Linux/Windows; requires trayIconEnabled. Off by default. |
...
| `graphApi.nextMeeting.lookaheadMinutes` | `number` | `30` | How far ahead (minutes) the next-meeting tray tooltip searches the calendar; clamped to 5-120. |
| `graphApi.nextMeeting.pollIntervalMs` | `number` | `60000` | Poll interval in milliseconds for the next-meeting tray tooltip calendar fetch; clamped to 15s-10m. |
| `graphApi.calendar.enabled` | `boolean` | `false` | Keep an in-memory calendar refreshed with Graph delta queries and enable the Tools &gt; Calendar panel; requires graphApi.enabled. |
| `graphApi.calendar.syncIntervalMs` | `number` | `300000` | Background calendar delta-sync interval in milliseconds; clamped to 30s-60m. |
| `graphApi.mailPreview.enabled` | `boolean` | `false` | Show a system notification (with history entry) for new inbox mail; requires graphApi.enabled and Microsoft Mail.Read consent. |
| `graphApi.mailPreview.pollIntervalMs` | `number` | `300000` | Inbox poll interval in milliseconds for mail preview notifications; clamped to 1m-30m. |
...
| `extensions.preload` | `array` | `[]` | Absolute paths to unpacked extension directories loaded at startup. |
| `extensions.identityShim.enabled` | `boolean` | `false` | Patch chrome.identity.launchWebAuthFlow and chrome.tabs.create on extension popup/options pages so OAuth sign-in flows can complete (off by default). |
| `extensions.identityShim.allowedRedirectHosts` | `array` | `["chromiumapp.org"]` | HTTPS hosts allowed as chrome.identity redirect targets (e.g. chromiumapp.org). |
| `extensions.identityShim.authCompleteHosts` | `array` | `["otter.ai"]` | Hosts that mark an OAuth sign-in as complete (e.g. otter.ai). When a sign-in window lands on one, loaded extensions are reloaded so they pick up the freshly-set session cookie immediately. |
