...
| `presence.graphPoll.intervalMs` | `number` | `60000` | Poll interval in milliseconds for Graph /me/presence when graphPoll is enabled. |
| `presence.keepAlwaysOnline` | `boolean` | `false` | Keep presence as Available even when the system is idle (forces active state reported to Teams and MQTT/tray). Disabled by default; lives under Debug in the menu. |
