...
| `linux` | `object` | `{"waylandMode":"auto","portal":{"enabled":true},"mediaControls":{"enabled":false}}` | Linux desktop integration. waylandMode selects automatic, forced, or disabled Wayland handling. portal.enabled prefers xdg-desktop-portal/PipeWire screen sharing when available and falls back to the Electron picker. mediaControls is an opt-in Linux media-control bridge. | `restart` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `linux.waylandMode` | `string` | `"auto"` | Wayland handling mode: auto detects the active session, enabled forces Wayland integration, disabled uses the legacy path. |
| `linux.portal.enabled` | `boolean` | `true` | Prefer xdg-desktop-portal/PipeWire for screen sharing when the active session supports it. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `wayland.mode` | `string` | `"auto"` | Compatibility alias for linux.waylandMode. |
| `wayland.portal.enabled` | `boolean` | `true` | Compatibility alias for linux.portal.enabled. |
