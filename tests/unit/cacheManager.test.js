...
describe('CacheManager directory scanning', () => {
  ...
  describe('cleanDirectory', () => {
    ...
    it('continues cleaning siblings when one entry cannot be removed', async () => {
      ...
      // Inject the failure instead of relying on directory permissions: the
      // latter are bypassed when the test runner runs as root (common in CI).
      const blockedPath = path.join(lockedDir, 'c.txt');
      ...
      try {
        ...
        assert.ok(fs.existsSync(blockedPath), 'locked entry contents remain');
      } finally {
